...
# post 글 작성
    ...
    def do_POST(self):
        if None != re.search('/api/v1/addrecord/*', self.path): #path는 부모에서 넣어주는것
            ...
            if ctype == 'application/json': # josn 형식이면 밑으로
                ...
                tempDict = json.loads(receivedData) #  load your str into a dict 즉 STR -> DICT로 !  json디코딩
...
except KeyboardInterrupt: # ctrl + c 누를때까지는! 계속 server 돈다!
